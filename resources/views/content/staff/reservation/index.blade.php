...
@section('content')
        ...
        <div class="card-body">
          <div class="d-flex justify-content-between align-items-center">
            <div>
              ...
              <p class="mb-0">View and manage guest bookings. Contact an admin to approve or reject.</p>
            </div>
          </div>
        </div>
      </div>
              ...
              <tbody id="bookingsTableBody">
                @forelse($bookings as $i => $bk)
                  ...
                  <tr class="booking-row"
                    ...
                    <td><span class="row-num text-body-secondary">{{ $i + 1 }}</span></td>
                    ...
                    <td><span class="fw-medium">{{ \Carbon\Carbon::parse($bk->check_in_date)->format('M d, Y') }}</span></td>
                    <td><span class="fw-medium">{{ \Carbon\Carbon::parse($bk->check_out_date)->format('M d, Y') }}</span></td>
                    ...
                    <td>
                      ...
                      @elseif($bk->payment_method === 'cash')
                        <span class="badge bg-label-warning"><i class="ri ri-cash-line me-1"></i>Cash</span>
                      @else
                    ...
                    <td>
                      ...
                      @elseif($overallSt === 'approved')
                        <span class="badge bg-label-success"><i class="ri ri-checkbox-circle-line me-1"></i>Approved</span>
                      @elseif($overallSt === 'pending')
                        <span class="badge bg-label-warning"><i class="ri ri-time-line me-1"></i>Pending</span>
                      @elseif($overallSt === 'rejected')
                        <span class="badge bg-label-danger"><i class="ri ri-close-circle-line me-1"></i>Rejected</span>
                      @else
                        <span class="badge bg-label-secondary"><i class="ri ri-forbid-line me-1"></i>Cancelled</span>
                      @endif
                    </td>
                    ...
                    <td>
                      <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-end">
                          ...
                          <a class="dropdown-item" href="javascript:void(0);"
                             onclick="viewBooking({{ $bk->primary_reservation_id }})">
                            <i class="icon-base ri ri-eye-line me-2"></i>View Details
                          </a>

                          {{-- Cancel — only when booking has active rooms --}}
                          @if($anyActive && $bk->payment_id)
                            <div class="dropdown-divider"></div>
                            <a class="dropdown-item text-warning" href="javascript:void(0);"
                               onclick="cancelBooking({{ $bk->payment_id }}, {{ json_encode($bk->reservation_ids) }}, '{{ addslashes($bk->first_name . ' ' . $bk->last_name) }}')">
                      ...
                      <div style="display:none;">
                        <input id="showUrl__{{ $bk->primary_reservation_id }}"
                               value="{{ route('staff.reservations.show', $bk->primary_reservation_id) }}">
                        @if($bk->payment_id)
                          <input id="bulkCancelUrl__{{ $bk->payment_id }}"
                                 value="{{ route('staff.reservations.booking.cancel', $bk->payment_id) }}">
                        @endif
                        @foreach($bk->reservation_ids as $rid)
                          <input id="cancelUrl__{{ $rid }}"
                                 value="{{ route('staff.reservations.cancel', $rid) }}">
                        @endforeach
                      </div>
                    </td>
  ...
  <style>
    ...
    .staff-readonly-notice {
      ...
      font-size:.75rem; color:#566a7f; background:#f4f5fa;
      border:1px solid #e0e3eb; border-radius:.35rem;
      padding:.25rem .75rem;
    }
  </style>

  <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
  <script src="{{ asset('js/reservationjs/staff.js') }}"></script>
@endsection