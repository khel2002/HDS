...
<style>
    ...
    .status-indicator-dot.available { background: var(--success); }
    .status-indicator-dot.occupied { background: var(--azure-primary); }
    .status-indicator-dot.maintenance { background: var(--warning); }
    .status-indicator-dot.reserved { background: #8C98AC; }
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="status-badge-modern {{
                                        $reservation->reservation_status == 'approved' ? 'success' :
                                        ($reservation->reservation_status == 'pending' ? 'warning' :
                                        ($reservation->reservation_status == 'rejected' ? 'danger' : 'info'))
                                    }}">
            ...
            <table class="modern-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @php
                                $rate = $roomType->total_rooms > 0 ? round(($roomType->occupied_rooms / $roomType->total_rooms) * 100, 1) : 0;
                            @endphp
                            <span class="status-badge-modern {{ $rate >= 75 ? 'success' : ($rate >= 50 ? 'warning' : 'danger') }}">
                                {{ $rate }}%
                            </span>
                        </td>
                    </tr>
            ...
            options: {
                ...
                scales: {
                    y: {
                        ...
                        grid: { color: '#F1F3F7', drawTicks: false },
                        ticks: {
                            padding: 8,
