...
@section('content')
      ...
      <div class="card">
        <div class="card-body">
          <div class="d-flex justify-content-between align-items-center">
            ...
            <a href="{{ route('admin.reservations.walkin') }}" class="btn btn-primary" type="button">
                    ...
                    <td>
                      ...
                      <div style="display:none;">
                        <input id="showUrl__{{ $bk->primary_reservation_id }}"
                               value="{{ route('admin.reservations.show', $bk->primary_reservation_id) }}">
                        @if($bk->payment_id)
                          <input id="bulkApproveUrl__{{ $bk->payment_id }}"
                                 value="{{ route('admin.reservations.booking.approve', $bk->payment_id) }}">
                          <input id="bulkRejectUrl__{{ $bk->payment_id }}"
                                 value="{{ route('admin.reservations.booking.reject', $bk->payment_id) }}">
                          <input id="bulkCancelUrl__{{ $bk->payment_id }}"
                                 value="{{ route('admin.reservations.booking.cancel', $bk->payment_id) }}">
                        @endif
                        @foreach($bk->reservation_ids as $rid)
                          <input id="approveUrl__{{ $rid }}"
                                 value="{{ route('admin.reservations.approve', $rid) }}">
                          <input id="rejectUrl__{{ $rid }}"
                                 value="{{ route('admin.reservations.reject', $rid) }}">
                          <input id="cancelUrl__{{ $rid }}"
                                 value="{{ route('admin.reservations.cancel', $rid) }}">
                          <input id="deleteUrl__{{ $rid }}"
                                 value="{{ route('admin.reservations.destroy', $rid) }}">
                        @endforeach
                      </div>
                    </td>
  ...
  <script src="{{ asset('js/reservationjs/admin.js') }}"></script>
@endsection