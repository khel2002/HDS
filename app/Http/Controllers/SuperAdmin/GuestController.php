...
class GuestController extends Controller
{
    ...
    public function all(Request $request)
    {
        ...
        $stats = [
            ...
            'active_guests'  => $guests->where('STATUS', 'active')->count(),
            'inactive_guests'=> $guests->where('STATUS', 'inactive')->count(),
        ...
        return view('content.super-admin.guests.all', compact('guests', 'stats', 'search', 'status'));
    }
    ...
    public function current(Request $request)
    {
        ...
        return view('content.super-admin.guests.current', compact(
    ...
    public function history(Request $request)
    {
        ...
        return view('content.super-admin.guests.history', compact(
    ...
    // ─────────────────────────────────────────────
    //  DESTROY  (soft-delete: set status = inactive)
    // ─────────────────────────────────────────────
    public function destroy($id)
    {
        ...
        $activeStay = DB::table('registrations')
            ->where('user_id', $id)
        ...
        if ($activeStay) {
            return response()->json([
                'success' => false,
                'message' => 'Cannot delete a guest who is currently checked in.',
            ], 422);
        }
        ...
        return response()->json([
            'success' => true,
            'message' => 'Guest account has been deactivated.',
        ]);
    }
}