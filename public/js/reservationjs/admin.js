...
function buildModalBody(bk) {
  ...
  const payStatusBadge = { completed:'bg-label-success', pending:'bg-label-warning', refunded:'bg-label-info' };
...
async function confirmDeleteBooking(reservationIds, guestName) {
  ...
  closeViewThen(async () => {
    ...
    try {
      ...
      document.getElementById(`deleteUrl__${reservationIds[0]}`)
        ?.closest('tr')?.remove();
...
function buildStatusBadge(status) {
  const cls = { approved:'bg-label-success', pending:'bg-label-warning', rejected:'bg-label-danger', cancelled:'bg-label-secondary' };
  const ico = { approved:'ri-checkbox-circle-line', pending:'ri-time-line', rejected:'ri-close-circle-line', cancelled:'ri-forbid-line' };
  return `<span class="badge ${cls[status] || 'bg-label-secondary'}"><i class="ri ${ico[status] || 'ri-question-line'} me-1"></i>${cap(status)}</span>`;
}
