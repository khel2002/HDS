/**
 * staff_orders_script.js
 * Staff-only breakfast order management.
 * Mirrors orders_script.js but scoped to staff routes — no menu management.
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ── Sync modal from row data on open ──────────────────────────────────
  function syncModalFromRow(orderId) {
    ...
    if (timelineEl) timelineEl.innerHTML = buildTimeline(currentStatus);
  ...
  function patchModal(item) {
    ...
    const timelineEl = modal.querySelector('.modal-timeline');
    if (timelineEl) timelineEl.innerHTML = buildTimeline(item.request_status);
  ...
  // ── Update stat counters & quick-filter pills ──────────────────────────
  function updateStats(stats) {
    statKeys.forEach(key => {
      const el   = document.querySelector(`[data-stat="${key}"]`);
      const pill = document.getElementById(`pill-${key}`);
      if (el   && stats[key] !== undefined) el.textContent   = stats[key];
      if (pill && stats[key] !== undefined) pill.textContent = stats[key];
    });
  }
